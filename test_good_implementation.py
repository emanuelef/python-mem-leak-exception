#!/usr/bin/env python3
"""
Test script demonstrating the fixed implementation with factory methods for exceptions.
"""

import sys
from memory_tracker import MemoryTracker
from good_implementation import RequestHandler
...
def run_test(num_requests=1000, user_exists=False):
    ...
    tracker = MemoryTracker("good_implementation")
    tracker.start()
    ...
    for i in range(num_requests):
        ...
        request_data = {
            ...
            "payload": {
                # Include a large payload to make memory usage patterns more visible
                "data": "x" * 1000,
    ...
    data_file = tracker.save_data(f"good_implementation_memory_{'success' if user_exists else 'error'}.csv")
    plot_file = f"good_implementation_memory_{'success' if user_exists else 'error'}.png"
    
    tracker.plot(
        title=f"Memory Usage with Factory Method Exceptions - {'Successful' if user_exists else 'Error'} Requests",
        save_path=plot_file
    )
...
if __name__ == "__main__":
    ...
    print("=== Testing GOOD Implementation (Factory Method Exceptions) ===")
    print(f"Running with {num_requests} requests that will raise exceptions...")
    
    # Test: Error path (will raise exceptions)
    ...
    print(f"Average change per request: {(final_memory - initial_memory) / num_requests * 1000:.2f} KB/request")
    
    print(f"\nCheck the plot at {error_plot} to verify stable memory usage")